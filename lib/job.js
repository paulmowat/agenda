...
Job.prototype.toJSON = function() { // create a persistable Mongo object -RR
    ...
    result.disabled = !!self.attrs.disabled;
...
Job.prototype.computeNextRunAt = function() {
  ...
  function computeFromInterval() {
    ...
    try {
      ...
      this.attrs.nextRunAt = nextDate.toDate();
    } catch (e) {
      ...
      try {
        if (repeatAt) {
          ...
          if (!this.attrs.lastRunAt && humanInterval(interval)) {
            this.attrs.nextRunAt = lastRun.toDate();
          } else {
  ...
  function computeFromRepeatAt() {
    ...
    var nextDate = dateForTimezone(date(repeatAt));

    var interval = interval || "1 days";

    var offset = dateForTimezone(Date.now());  // if you do not specify offset date for below test it will fail for ms
    if (offset.valueOf() === dateForTimezone(date(repeatAt, offset)).valueOf()) {
      this.attrs.nextRunAt = undefined;
      this.fail('failed to calculate repeatAt time due to invalid format');
    } else if (nextDate.valueOf() === lastRun.valueOf()) {
      this.attrs.nextRunAt = new Date(dateForTimezone(date(repeatAt)).valueOf() + humanInterval(interval));
    } else {
      this.attrs.nextRunAt = dateForTimezone(date(repeatAt)).toDate();
    }
  }
};
...
Job.prototype.repeatAtEvery = function(time, interval, options) {
  this.repeatAt(time);
  this.repeatEvery(interval, options);
  return this;
};
...
Job.prototype.schedule = function(time, options) {
  options = options || {};
  this._scheduled = true;
  var timezone = options.timezone ? options.timezone : null;
  this.attrs.nextRunAt = (time instanceof Date) ? time : date(time);

  if (timezone) {
    var d = moment(this.attrs.nextRunAt).tz(timezone);
    this.attrs.nextRunAt = d.toDate();
  }
